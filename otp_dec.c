...
int main(int argc, char *argv[]){
	...
	char *otp_dec_msg = "8 otp_dec\0";
	int otp_dec_msg_size = 10;
	...
	size_t ciphertext_filesize, cipherkey_filesize;
	...
	send_all(sockfd, otp_dec_msg, &otp_dec_msg_size);
	...
	if(strcmp(verify_status, "FAIL\0") == 0){
		fprintf(stderr, "Error! Only \'otp_dec\' allowed to access \'otp_dec_d\'.\n");
		close(sockfd);
		exit(1);
	}
	...
	// get plaintext file
	input_fstream = fopen(ciphertext_filename, "r");
	...
	// get filesize of ciphertext
	ciphertext_filesize = get_file_size(ciphertext_filename);


	// allocate memory to hold ciphertext
	ciphertext_msg = malloc(ciphertext_filesize * sizeof(char));
	...
	get_file(input_fstream, ciphertext_msg, ciphertext_filesize);
	
	// close ciphertext filestream
	fclose(input_fstream);

	if(check_keys(ciphertext_msg, ciphertext_filesize) == -1){
	...
	if(ciphertext_filesize > cipherkey_filesize){
	...
	// send plaintext file
	send_file(sockfd, ciphertext_msg, ciphertext_filesize);
	...
	plaintext_msg_length = get_msg_length(sockfd);
	...
	memset(plaintext_msg, '\0', plaintext_msg_length);
	recv_all(sockfd, plaintext_msg, plaintext_msg_length);

	printf("%s", plaintext_msg);
	fflush(stdout);	
