...
** Description: Daemon script that takes ciphertext and a cipher key sent
** over a socket connection from otp_dec script and decrypts
** Author: Ryan Ellis
...
int decrypt(char *ciphertext_msg, int ciphertext_msg_length, char *cipherkey_msg, char *plaintext_msg);
...
int main(int argc, char *argv[]){
	...
	while(1){
		...
		if (!fork()) { // this is the child proces!)s
			...
			// check verify msg
			if(strcmp(verify_msg, "otp_dec\0") != 0){
			...
			// get length of ciphertext msg incoming
			ciphertext_msg_length = get_msg_length(new_fd);

			if(ciphertext_msg_length == -1){
				close(new_fd);
			}

			// allocate enough mem for ciphertext msg
			...
			// get ciphertext msg
			recv_all(new_fd, ciphertext_msg, ciphertext_msg_length);
			...
			// decrypt msg
			plaintext_msg_length = ciphertext_msg_length;
			...
			if(decrypt(ciphertext_msg, ciphertext_msg_length, cipherkey_msg, plaintext_msg) == 1){
				send_all(new_fd, fail_msg, &fail_msg_length);
				close(new_fd);
			}
			else{
			...
			send_file(new_fd, plaintext_msg, plaintext_msg_length);
...
// Description: Decrypts ciphertext file using cipherkey provided
// Arguments: The ciphertext msg, the length of the ciphertext, the cipherkey,
// and space to hold the plaintext
// Returns: Returns 1 if invalid character was used in the plaintext, otherwise 0
int decrypt(char *ciphertext_msg, int ciphertext_msg_length, char *cipherkey_msg, char *plaintext_msg){

	// var setup
	int i;    // traveler
	int ciphertext_key_num, cipherkey_key_num, cipher_cipher_dif;
	char plaintext_char;

	for(i = 0; i < ciphertext_msg_length - 1; i++){
		
		// get plaintext num
		if((ciphertext_key_num = keyToNum(ciphertext_msg[i])) == -1){
			printf("Error! Key not found.\n");
			return 1;
		}

		// get cipherkey num
		if((cipherkey_key_num = keyToNum(cipherkey_msg[i])) == -1){
			printf("Error! Key not found.\n");
			return 1;
		}

		// total of plaintext cipher nums
		cipher_cipher_dif = ciphertext_key_num - cipherkey_key_num;

		if(cipher_cipher_dif < 0){
			cipher_cipher_dif += 27;
		}
	
		// get char for encrypted key
		plaintext_char = numToKey(cipher_cipher_dif);

		plaintext_msg[i] = plaintext_char;
